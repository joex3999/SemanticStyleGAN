...
def save_network(net, label, which_iter, opt, latest=False):
    if latest:
        save_path = os.path.join(
            opt.checkpoint_path, f"{label}_latest_net_{which_iter}.pth"
        )
...
def load_network(net, label, opt, override_iter=None, override_load_path=None):
    ...
    if load_path is not None and which_iter > 0:
        latest_load_path = os.path.join(
            load_path, f"{label}_latest_net_{which_iter}.pth"
        )
        load_path = os.path.join(load_path, f"{label}_net_{which_iter}.pth")
        if not os.path.exists(load_path):
            if not os.path.exists(latest_load_path):
                raise ValueError(
                    f"No checkpoint for {label} net at iter {which_iter} and path {load_path}"
                )
            else:
        ...
        load_paths = glob(
            os.path.join(
                ...
                "checkpoints",
                f"*-{opt.name}",
                f"{label}_latest_net_{which_iter}.pth",
            )
        )
        load_paths += glob(
            os.path.join(
                ...
                "checkpoints",
                f"*-{opt.name}",
                f"{label}_net_{which_iter}.pth",
            )
        )
        assert (
            len(load_paths) > 0
        ), f"Did not find any checkpoint for {label} net at iter {which_iter} and name {opt.name}"
        assert (
            len(load_paths) == 1
        ), f"Too many checkpoint candidates for {label} net at iter {which_iter} and name {opt.name}:\n{load_paths}"
