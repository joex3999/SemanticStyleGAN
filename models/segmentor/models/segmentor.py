...
from models.segmentor.models import load_network, save_network, print_network
...
class Segmentor(torch.nn.Module):
    def __init__(
        self, opt, is_train=True, is_main=True, logger=None, distributed=False
    ):
        super().__init__()
        self.opt = opt
        ...
        self.netS, self.netD_final, self.netD_inter = self.initialize_network(
            is_train, distributed=distributed
        )
        height = (
            self.opt.fixed_crop[0]
            if self.opt.fixed_crop is not None
            else int(self.opt.dim)
        )
        width = (
            self.opt.fixed_crop[1]
            if self.opt.fixed_crop is not None
            else int(self.opt.dim * self.opt.aspect_ratio)
        )
        ...
        if is_train:
            ...
            if self.opt.ohem:
                segmentation_loss = CriterionOhemDSN(
                    ignore_index=self.ignore_index,
                    thresh=self.opt.ohem_thres,
                    min_kept=self.opt.ohem_keep,
                    reduction="none",
                )
            else:
                segmentation_loss = CriterionDSN(
    ...
    def forward(
        ...
        if mode == "segmentor":
            loss, pred_seg, delta = self.compute_segmentation_loss(
                src_sem_seg, img, hard, log, global_iteration, suffix
            )
            return loss, pred_seg, delta

        if mode == "segmentor_advent":
            loss, pred_seg, delta = self.compute_advent_segmentation_loss(
                src_sem_seg, img, tgt_img, hard, log, global_iteration
            )
            return loss, pred_seg, delta

        if mode == "discriminator_advent":
            loss = self.compute_advent_discriminator_loss(
                pred_seg, pred_seg_tgt, log, global_iteration
            )
            return loss
    ...
    def preprocess_input(self, data, is_pred=False):
        if is_pred:
            data["sem_seg"] = (
                data["sem_seg"].detach() if "sem_seg" in data else torch.Tensor([])
            )
            data["sem_seg_inter"] = (
                data["sem_seg_inter"].detach()
                if "sem_seg_inter" in data
                else torch.Tensor([])
            )
            data["sem_seg_tgt"] = (
                data["sem_seg_tgt"].detach()
                if "sem_seg_tgt" in data
                else torch.Tensor([])
            )
            data["sem_seg_inter_tgt"] = (
                data["sem_seg_inter_tgt"].detach()
                if "sem_seg_inter_tgt" in data
                else torch.Tensor([])
            )
            pred_seg = {
                "sem_seg": data["sem_seg"],
                "sem_seg_inter": data["sem_seg_inter"],
            }
            pred_seg_tgt = {
                "sem_seg": data["sem_seg_tgt"],
                "sem_seg_inter": data["sem_seg_inter_tgt"],
            }
            return pred_seg, pred_seg_tgt
        else:
            data["img"] = data["img"].cuda() if "img" in data else torch.Tensor([])
            data["sem_seg"] = (
                data["sem_seg"].cuda() if "sem_seg" in data else torch.Tensor([])
            )
            img = data["img"]
    ...
    def initialize_network(self, is_train, distributed=False):
        ...
        print(self.opt)
        num_classes = (
            len(self.opt.eval_idx)
            if self.opt.segment_eval_classes_only
            else self.opt.num_semantics
        )
        ...
        if is_train and self.opt.advent:
            ...
            netD_inter = (
                get_fc_discriminator(num_classes=num_classes).cuda()
        ...
        if self.is_main:
            ...
            if is_train and self.opt.advent:
                ...
                if self.opt.advent_multi:
                    if self.opt.cont_train:
                        netD_inter = load_network(
                            netD_inter, "advent_d_inter", self.opt
                        )
    ...
    def compute_segmentation_loss(
        self, src_sem_seg, img, hard, log, global_iteration, suffix
    ):
        pred_seg = self.predict(
            img=img, hard=False, log=False, global_iteration=global_iteration
        )
        flat_src = self.flatten_sem(src_sem_seg)
        unreduced_loss = self.segmentation_loss(
            [
                self.reduce_sem(pred_seg["sem_seg"]),
                self.reduce_sem(pred_seg["inter_sem_seg"]),
            ],
            flat_src,
        )
        ...
        if self.logger:
            ...
            self.logger.log_scalar(
                f"segmentor/segmentation{suffix}", loss.cpu(), global_iteration
            )
            # log images every few steps
            if log:
                self.logger.log_semantic_seg(
                    f"segmentor/real{suffix}",
                    src_sem_seg[:16].float().cpu(),
                    4,
                    global_iteration,
                )
                self.logger.log_semantic_seg(
                    f"segmentor/pred{suffix}",
                    pred_seg["sem_seg"][:16].float().cpu(),
                    4,
                    global_iteration,
                )
                self.logger.log_img(
                    f"segmentor/img{suffix}",
                    img[:16].float().cpu(),
                    4,
                    global_iteration,
                    normalize=True,
        ...
        if hard:
            ...
            x_pred_sem_seg = torch.zeros_like(pred_seg["sem_seg"]).scatter_(
                1, index, 1.0
            )
            return (
                loss,
                {"sem_seg": x_pred_sem_seg, "inter_sem_seg": pred_seg["inter_sem_seg"]},
                delta,
            )
        else:
            return loss, pred_seg, delta

    def compute_advent_segmentation_loss(
        self, src_sem_seg, img, tgt_img, hard, log, global_iteration
    ):
        src_label = 0
        ...
        if self.opt.advent_multi:
            for param in self.netD_inter.parameters():
                param.requires_grad = False
        # train with tgt
        pred_seg_tgt = self.predict(
            img=tgt_img, hard=False, log=False, global_iteration=global_iteration
        )
        if self.opt.advent_multi:
        ...
        loss_adv = (
            self.opt.advent_lambda_adv_final * loss_adv_tgt_final
        ...
        if self.logger:
            ...
            self.logger.log_scalar(
                "segmentor/advent_adv_inter", loss_adv_tgt_inter, global_iteration
            )
            self.logger.log_scalar(
                "segmentor/advent_adv_final", loss_adv_tgt_final, global_iteration
            )
            self.logger.log_scalar("segmentor/advent_adv", loss_adv, global_iteration)
            self.logger.log_scalar(
                "segmentor/advent_ent_tgt", torch.mean(ent_tgt), global_iteration
            )
            # log images every few steps
            if log:
                self.logger.log_img(
                    "segmentor/entropy_tgt",
                    ent_tgt[:16].float().cpu(),
                    4,
                    global_iteration,
                )
                self.logger.log_semantic_seg(
                    "segmentor/src", src_sem_seg[:16].float().cpu(), 4, global_iteration
                )
                self.logger.log_semantic_seg(
                    "segmentor/pred_tgt",
                    pred_seg_tgt["sem_seg"][:16].float().cpu(),
                    4,
                    global_iteration,
                )
                self.logger.log_semantic_seg(
        ...
        if hard:
            ...
            pred_data = {
                "sem_seg": x_pred_sem_seg,
                "inter_sem_seg": pred_seg["inter_sem_seg"],
            }
        else:
            pred_data = pred_seg
    ...
    def compute_advent_discriminator_loss(
        self, pred_seg, pred_seg_tgt, log, global_iteration
    ):
        src_label = 0
        tgt_label = 1
        ...
        if self.logger:
            # log scalars every step
            self.logger.log_scalar(
                "segmentor/advent_dis_inter", loss_dis_inter, global_iteration
            )
            self.logger.log_scalar(
                "segmentor/advent_dis_final", loss_dis_final, global_iteration
            )
            self.logger.log_scalar("segmentor/advent_dis", loss_d, global_iteration)
            self.logger.log_scalar(
                "segmentor/advent_ent_src", torch.mean(ent_src), global_iteration
            )
            if log:
                self.logger.log_img(
                    "segmentor/entropy_src", ent_src[:16].cpu(), 4, global_iteration
                )
    ...
    def predict(self, img, hard, log, global_iteration):
        ...
        if self.logger and log:
            self.logger.log_semantic_seg(
                "segmentor/pred", preds[0][:16], 4, global_iteration
            )
