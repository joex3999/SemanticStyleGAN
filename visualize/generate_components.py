...
if __name__ == "__main__":
    ...
    if args.dataset_name == "cityscapes":
        component_dict = component_dict_cityscapes_v2
    else:
    ...
    with torch.no_grad():
        ...
        print(f'Shape of latent is {styles.shape}')
        for sample_index in range(styles.size(0)):
            ...
            print(f'Shape of composition_mask is {composition_mask.shape}')
            sample_outdir = (
                args.outdir
                if args.latent is not None
                else f"{args.outdir}/{sample_index}"
            )
            ...
            for component_index, component_name in component_dict.items():
                ...
                image, seg, seg_coarse, depths, _ = model(
                    [style_inputs],
                ...
                imageio.imwrite(
                    f"{sample_outdir}/image_{component_index}_{component_name}.jpg",
                    image[0],
                )
                imageio.imwrite(
                    f"{sample_outdir}/seg_{component_index}_{component_name}.jpg",
                    seg[0],
                )
                imageio.imwrite(
                    f"{sample_outdir}/seg_coarse_{component_index}_{component_name}.jpg",
                    seg_coarse[0],
                )
                visualize_alpha(
                    f"{sample_outdir}/depth_{component_index}_{component_name}.jpg",
                    depths[component_index][0],
                )
