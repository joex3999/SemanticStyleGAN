    ...
    for idx in pbar:
        ...
        if get_rank() == 0:
            if i % 100 == 0:
                ...
                if writer is not None:
                    writer.add_scalar(
                        "scores/real_score", real_score_val, global_step=i
                    )
                    writer.add_scalar(
                        "scores/fake_score", fake_score_val, global_step=i
                    )
                    ...
                    writer.add_scalar(
                        "path/path_length", path_length_val, global_step=i
                    )
            ...
            if i % args.viz_every == 0:
                with torch.no_grad():
                    g_ema.eval()
                    sample_img, sample_seg, sample_seg_coarse, depths, _ = g_ema(
                        [sample_z], return_all=True
                    )
                    sample_img = sample_img.detach().cpu()
                    sample_mask = color_segmap(sample_seg.detach().cpu(), color_map)
                    sample_mask_coarse = color_segmap(
                        sample_seg_coarse.detach().cpu(), color_map
                    )
